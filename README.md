...
Drop the `DevouringWatch` folder into `World of Warcraft\_retail_\Interface\AddOns\`, or install via the CurseForge app.

---

## Usage

DevouringWatch works without any configuration. Log in, fly to Eversong or Zul'Aman, and the panel automatically detects and displays active Void Assault events. Use `/dw scan` to see raw POI IDs if you want to report them or verify detection.
...
Uses `C_AreaPoiInfo` and `C_QuestLog`, both fully accessible in Midnight. `C_AreaPoiInfo.GetAreaPOIInfo` is always called with the required `mapID`.
...
Open an issue on [GitHub](https://github.com/Nelnamara/DevouringWatch/issues) or leave a CurseForge comment.
